...
/**
 * 基于物品的协同推荐系统（采用改进后的余弦相似度）跟ItemCFRecommenderThread内容一样 测试使用的
 */
public class ItemCFRecommender1 {

    public static double[][] itemsCorrelationCoefficient() throws ExecutionException, InterruptedException {
        ...
        List<Future<double[][]>> itemFutureList = new LinkedList<>();
        ...
        ItemRunnable.moviesIdListLength = moviesIdList.size();

        int nums = moviesIdList.size() / 3;
        int start = 0;
        int end = -1;
        for (int i = 0; i < 2; i++) {
            start = end + 1;
            if (i == 1) end = moviesIdList.size() - 2;
            else end = start + nums;
            Future<double[][]> items = executor.submit(
                    new ItemRunnable(start, end));
            itemFutureList.add(items);
        }
        executor.shutdown();
...
//                    }
//                }
//                if (count1 != userIdList.size() && count2 != userIdList.size()) {
...
//        }
        ...
        for (Future<double[][]> items : itemFutureList) {
            item_item = items.get();
//            double[][] item_item1 = items.get();
//            for (int i = 0; i < item_item1.length; i++) {
//                for (int j = 0; j < item_item1[i].length; j++) {
//                    if(item_item1[i][j] != 0) item_item[i][j] = item_item1[i][j];
//                }
//            }
        }
     ...
     */
    public static Map<Integer, Double> itemCFRecommender(int userId) throws ExecutionException, InterruptedException {
